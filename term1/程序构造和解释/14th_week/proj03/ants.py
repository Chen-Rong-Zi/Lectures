...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        walk = self.place
        pos = 0
        while not walk.is_hive:
            if walk.bees and (self.min_range <= pos <= self.max_range):
                return random_bee(walk.bees)  # REPLACE THIS LINE
            walk = walk.entrance
            pos += 1
        return None
        # END PROBLEM 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown <= 0 and len(self.place.bees) != 0:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_waterproof and insect is not None:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN PROBLEM 12
        def behind_ants(place):
            if place is None:
                return [None]
            else:
                return [place.ant] + behind_ants(place.exit)

        super().action(gamestate)
        ants = behind_ants(self.place.exit)
        for ant in ants:
            if ant is None or isinstance(ant, QueenAnt): continue
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow(self, lenth):
            if not hasattr(self, 'slow_count_down'):
                self.slow_count_down = lenth
                return
            self.slow_count_down += lenth

        def action(self, gamestate):
            should_slow = lambda bee: hasattr(bee, 'slow_count_down') and bee.slow_count_down > 0

            if should_slow(self) and gamestate.time % 2 == 1:
                self.slow_count_down -= 1

            elif should_slow(self) and gamestate.time % 2 == 0:
                self.slow_count_down -= 1
                type(self).action(self, gamestate)

            elif not should_slow(self):
                type(self).action(self, gamestate)

        if target:
            target.slow = slow
            def combine_action(self_reference, target_action):
                def inner(gamestate):
                    target_action(gamestate)
#                     print(self_reference.place.name)
                    if self_reference.back_countdown != 0:
                        return
                    action(self_reference, gamestate)
                return inner

            if hasattr(target, 'back_countdown') and target.back_countdown != 0:
                target.action = combine_action(target, target.action)
            else:
                target.action = (lambda self_reference: lambda gamestate: action(self_reference, gamestate))(target)
            target.slow(target, 3)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN PROBLEM EC
        def back(bee, gamestate):
            if hasattr(bee, 'slow_count_down') and bee.slow_count_down != 0 and gamestate.time % 2 == 1:
                return
            bee.back_countdown -= 1
            if bee.place.ant:
                type(bee).action(bee, gamestate)
            if not bee.place.entrance.is_hive:
                bee.place.entrance.add_insect(bee)

        been_scared = lambda bee: hasattr(bee, 'scared')
        def action(self, gamestate):
            if been_scared(self) and self.back_countdown != 0:
                back(self, gamestate)
            else:
                type(self).action(self, gamestate)
        ...
        target.action = (lambda self_reference: lambda gamestate: action(self_reference, gamestate))(target)
        target.scared = True
        if not hasattr(target, 'back_countdown'):
            target.back_countdown = 2
        "*** YOUR CODE HERE ***"
        # END PROBLEM EC
