...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        operands = rest.map((lambda environment: lambda expression: scheme_eval(expression, environment))(env))
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        while args is not nil:
            ...
            args = args.rest
        if procedure.expect_env: py_args += [env]
        try:
...
def optimize_tail_calls(origin_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Unevaluated):
            result = origin_scheme_eval(result.expr, result.env)
        return result
        # END PROBLEM EC
