...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    return get_name(city1) if distance(make_city('', lat, lon), city1) <= distance(make_city('', lat, lon), city2) else get_name(city2)
...
def my_reduce(combiner, seq):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(combiner, seq, index):
        #         print(f"combiner = {combiner}, seq = {seq}, index = {index}")
        if index == 0:
            return seq[index]
        return combiner(helper(combiner, seq, index - 1), seq[index])
    return helper(combiner, seq, len(seq)-1)
