...
def count_k(n, k):
    ...
    """
    "*** YOUR CODE HERE ***"
    helper = lambda n, k, times : 0 if times == 0 else count_k(n-times, k) + helper(n, k, times-1)
    def helper(n, k, times=0):
        print(f"n = {n}, k = {k}, times = {times}")
        if times == 0:
            return 0
        else:
            return count_k(n-times, k) + helper(n, k, times-1)
    ...
    if (n == 1 or n == 0 or k == 1):
        ...
        return helper(n, k, k)
#     return 1 if (n == 1 or n == 0 or k == 1) else count_k(n, n) if n < k else helper(n, k, k)
...
def max_subseq(n, l):
    ...
    if l == 0:
        return 0
    elif n < 10:  # equivalent to (n < 10) and (l != 0)
        return n
    else:
