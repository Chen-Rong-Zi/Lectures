...
def accumulate(combiner, base, n, f):
    ...
    def helper(n, so_far):
        if n == 0:
            return so_far
        return helper(n - 1, combiner(f(n), so_far))
...
def summation_using_accumulate(n, f):
    ...
    """
    "*** YOUR CODE HERE ***"
    return accumulate(lambda x, y: x+y, 0, n, f)
...
def product_using_accumulate(n, f):
    ...
    """
    "*** YOUR CODE HERE ***"
    return accumulate(lambda x, y: x*y, 1, n, f)
...
def zero(f):
    return lambda x: x # add zero function applied to a parameter
...
def add_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda x:lambda y: m(x)(n(x)(y))
...
def mul_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    if church_to_int(n) == 0:
        return zero
    return lambda x : lambda y : add_church(m, mul_church(m, prev_church(n)))(x)(y)


def prev_church(n):
    def inner(f):
        def inner2(x):
            return n(lambda g : lambda h : h(g(f)))(lambda u : x)(lambda x : x)
        return inner2
    return inner
...
def pow_church(m, n):
    ...
    if church_to_int(n) == 0:
        ...
        return lambda x: lambda y: mul_church(m, pow_church(m, prev_church(n))) (x)(y)
