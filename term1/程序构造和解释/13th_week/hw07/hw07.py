...
class Polynomial:
    ...
    def __str__(self):
        ...
        for i in range(1, len(self.lst)):
            result += f" + {self.lst[i]}*x^{i}"
        return result

    def __add__(self, other):
        min_lst, max_lst = (list(self.lst), list(other.lst)) if len(self.lst) < len(other.lst) else (list(other.lst), list(self.lst))
        for i in range(len(min_lst)):
            max_lst[i] += min_lst[i]
        return Polynomial(max_lst)

    def __mul__(self, other):
        lst = [0 for i in range(len(other.lst) + len(self.lst))]
        for i in range(len(other.lst)):
            for j in range(len(self.lst)):
                lst[j + i] += other.lst[i] * self.lst[j]
...
def remove_duplicates(lnk):
    ...
    Link(1, Link(5))
    """
    if  (lnk is Link.empty) or (lnk.rest is Link.empty): return
...
def reverse(lnk):
    ...
    if lnk.rest is Link.empty: return lnk
    head = reverse(lnk.rest)
    third = head
    while third.rest != Link.empty:
        third = third.rest
    third.rest = lnk
    lnk.rest = Link.empty
    return head
...
def count_coins_tree(total, denominations):
                            ...
                            3, [1, 5, 10, 25]
                              2, [1, 5, 10, 25]
                                1, [1, 5, 10, 25]
                                  1
    """
    def helper(total, denominations):
        if total in denominations and count_coins(total, denominations) == 1:
            return [Tree(1)]

        without_current = with_current = None
        if count_coins(total, denominations[1:]) > 0:
            without_current = Tree(f"{total}, {denominations[1:]}", helper(total, denominations[1:]))
        if count_coins(total - denominations[0], denominations) > 0:
            with_current = Tree(f"{total - denominations[0]}, {denominations}", helper(total - denominations[0], denominations))
    ...
    if not denominations:
        return None
    result = helper(total, denominations)
    if len(result) == 2:
        return Tree(f'{total}, {denominations}', [result[0], result[1]])
    else:
        return Tree(f'{total}, {denominations}', [result[0]])
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        result = True
        for b in t.branches:
            result = result and is_bst(b)
        return result

    elif len(t.branches) == 2:
        result = (t.branches[0].label <= t.branches[1].label) and (t.branches[0].label <= t.label) and (t.branches[1].label > t.label)
        if result == False:
            return False
        else:
            result = True
