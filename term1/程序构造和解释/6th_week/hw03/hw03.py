...
def number_of_six(n):
    ...
    if n <= 0:
        return 0
    return (n % 10 == 6) + number_of_six(n // 10)
...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(target, sequence, result, step):
        if sequence == target:
            return result
        if (sequence+1) % 6 == 0 or haveSix(sequence+1):
            return helper(target, sequence+1, result+step, -step)
        else:
            return helper(target, sequence+1, result+step, step)
...
def missing_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(before, now, sum, n):
        # print(f"before = {before}, now = {now}, sum = {sum}, n = {n}")
        now = n % 10
        if n == 0:
            return sum
        if now != before:
            return helper(now, now, sum + before - now -1, n//10)
        elif now == before:
            return helper(now, now, sum, n//10)
    return helper(n%10, 0, 0, n)
...
def count_change(total, next_money):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(total, money, moneyfunc):
        ...
        if total < money:
            return 0
        return helper(total - money, money, moneyfunc) + helper(total, moneyfunc(money), moneyfunc)
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    def helper(n, start, end, times):
        if n <= 1:
            print_move(start, end)
        if start == end or n == 1:
            return
        helper(n-1, start, 6-start-end, times+1)
        helper(1, start, end, times+1)
        helper(n-1, 6-start-end, end, times+1)
    return helper(n, start, end, 0)
...
def multiadder(n):
    ...
    "*** YOUR CODE HERE ***"
    def outer(n, sum):
        def helper(x, sum = sum):
            if n == 1:
                return x + sum
            return outer(n-1, sum + x)
        return helper
    return outer(n, 0)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: (lambda x: x(x)) (lambda x: f(lambda z: x(x)(z) ) )) (lambda c: lambda z: 1 if z == 1 else z * c(z-1))


Y = lambda f: (lambda x: x(x)) (lambda x: f(lambda z: x(x)(z)))
fib_maker           = lambda f: lambda r: 1 if r == 1 else 0  if r  == 0  else f(r-1)   + f(r-2)
