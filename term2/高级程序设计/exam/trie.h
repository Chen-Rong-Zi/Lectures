...
class TrieNode {
  public:
    ...
    // Clone 函数返回当前 TrieNode
    // 的一个拷贝，如果其含有值也会拷贝对应值。它会返回一个指向拷贝后内容的
    // unique_ptr
    //
    // 由于拷贝构造函数无法判断当前类是否是子类型 TrieNodeWithValue,
    // 所以你不能使用拷贝构造函数拷贝节点
    //
    // 你可以使用 `std::shared_ptr<T>(std::move(ptr))` 将 `unique_ptr` 转化为
    // `shared_ptr`
    virtual std::unique_ptr<TrieNode> Clone() const {
        return std::make_unique<TrieNode>(children_);
    }

    // 一个子节点的 map，其中的键是下一个字母，对应值是指向的子节点
...
class TrieNodeWithValue : public TrieNode {
  public:
    ...
    // 根据已有的子节点和值创建节点
    TrieNodeWithValue(std::map<char, std::shared_ptr<const TrieNode>> children, std::shared_ptr<int> value):
    ...
    // 保存的值
    // 你可以通过 make_shared<int> 来构造它
    std::shared_ptr<int> value_;
};

// 从总体上来说，字典树是一个从 string 到 int * 的映射
// 在实现 Trie 的接口时，你需要尽可能地重用已有的 TrieNode
// 并在必要时创建新的节点
//
// 请不要删除 const 或添加 mutable 修饰符
class Trie {
private:
    // 指向了字典树的子节点
