...
def picky_piggy(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    leftNum = opponent_score % 100
    return abs(leftNum % 10 - leftNum // 10) * 2 + 1
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
...
def play(strategy0, strategy1,
    ...
    while True:
        ...
        scores[who] += take_turn(strategies[who](scores[who], scores[other(who)]), scores[other(who)], dice)
#         say = say(scores[0], scores[1])
        if swine_swap(scores[who]):
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    return lambda current_score, opponent_score: n
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    maxNum = 0
    for i in range(1, 11):
        currNum = make_averaged(roll_dice, total_samples)(i, dice)
        if currNum > maxNum:
            maxNumIndex = i
            maxNum = currNum
    return maxNumIndex
...
def swine_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if (swine_swap(score + picky_piggy(opponent_score)) and opponent_score >= score + threshold):
        return 0
    if (picky_piggy(opponent_score) >= threshold) and not (swine_swap(score + picky_piggy(opponent_score))):
        return 0
    return num_rolls
    # END PROBLEM 11
