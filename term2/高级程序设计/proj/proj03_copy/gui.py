...
INSECT_FILES = {
        ...
        'Scuba':        ASSETS_DIR + INSECT_DIR + "ant_scary.gif",
        ...
        'ScubaThower':        ASSETS_DIR + INSECT_DIR + './ant_scuba.gif',
...
class HttpHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        ...
        # print(format.replace('%s', '{}').format(*args))
        return
    ...
    def get_state(self, data):
        res = post(f'http://127.0.0.1:8090/ajax/fetch/state/{self.client_address[0]}', data=data).json()
        print(res)
        return res
    def start_game(self, data):
        return post(f'http://127.0.0.1:8090/ajax/start/game/{self.client_address[0]}', data=data)
    def exit_game(self, data):
        return post(f'http://127.0.0.1:8090/ajax/exit/{self.client_address[0]}', data=data).json()
    def deploy_ant(self, data):
        response = None
        try:
            response = post(f'http://127.0.0.1:8090/ajax/deploy/ant/{self.client_address[0]}', data=data)
            print(f'http://127.0.0.1:8090/ajax/deploy/ant/{self.client_address[0]}')
    ...
    def do_POST(self):
        if self.client_address[0] not in HttpHandler.ip_pool:
            print(self.client_address)
            HttpHandler.ip_pool.append(self.client_address[0])
            with open('ip.log', 'a') as f:
                f.write(str(self.client_address) + '\n')
        path = self.path
        action = {
                '/ajax/fetch/state': lambda data : HttpHandler.get_state(self, data),
                '/ajax/start/game': lambda data : HttpHandler.start_game(self, data),
                '/ajax/exit': lambda data : HttpHandler.exit_game(self, data),
                '/ajax/deploy/ant': lambda data : HttpHandler.deploy_ant(self, data)
                }.get(path)
        # action = {
        ...
        if response:
            ...
            self.end_headers()
            response = json.dumps(response) if isinstance(response, dict) else ''
