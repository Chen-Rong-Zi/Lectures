...
function startGame() {
    ...
    gui.interval = setInterval(gui.update.bind(gui), 1000);
}
...
GUI.prototype.moveBees = function() {
    ...
    // gameWrapper position
    gw_position = $("#gameWrapper").position()
    for (bee in newLocation) {
        if (oldLocation[bee] != newLocation[bee]) {
            ...
            if (img.css("position") != "absolute") {
                ...
                currentLocTop = img.position().top - gw_position.top;
                currentLocLeft = img.position().left - gw_position.left;
                img.css({"margin-top": "40px", "top": currentLocTop, "left": currentLocLeft, "position": "absolute"});
            }
            position = loc.position();
            target_position = {
                top: position.top-gw_position.top,
                left: position.left-gw_position.left
            }
            img.animate(target_position, 1000);
        }
    }
