...
def scheme_read(src, level=0):
    ...
    (car (quote (1 2)))
    """
    print('    ' * level + 'in scheme_read ' + str(src.current()))
    ...
    # elif val == "'":
        # "*** YOUR CODE HERE ***"
        # return Pair('quote', Pair(scheme_read(src, level + 1), nil))
    elif val == "(":
        return read_tail(src, level + 1)
    else:
        raise SyntaxError("unexpected token: {0}".format(val))

def read_tail(src, level=0):
    ...
    """
    try:
        print("    " * level + "in read_tail " + str(src.current()))
        if src.current() is None:
        ...
        # elif src.current() == ".":
            ...
            first = scheme_read(src, level + 1)
            rest = read_tail(src, level + 1)
